...
for i in b2b_in_store:
    ...
    neg_param = [b['option']['text'] for a in i['detail'] if a['id'] == 1801
                 for b in a['options'] if i['questionnaire'] == 122]
    ...
    neg_detail = [b['other_text'] for a in i['detail'] if a['id'] == 1801
                  for b in a['options'] if i['questionnaire'] == 122]
    # prod_not_received = [b['other_text'] for a in i['detail'] if a['id'] == 1818
    #                      for b in a['options'] if i['questionnaire'] == 122]
    cm_relationship = [b['option']['text'] for a in i['detail'] if a['id'] == 1802
                       for b in a['options'] if i['questionnaire'] == 122]
    rm_visit_frequency = [b['option']['text'] for a in i['detail'] if a['id'] == 1803
                          for b in a['options'] if i['questionnaire'] == 122]
    continue_business = [b['option']['text'] for a in i['detail'] if a['id'] == 1804
                         for b in a['options'] if i['questionnaire'] == 122]
    ...
    data_dict = {
        ...
        "How is your professional relationship with customer manager": ", ".join(cm_relationship)
        if cm_relationship is not None else "",
        "What is the visiting frequency our relationship manager visits you": ", ".join(rm_visit_frequency) if
        rm_visit_frequency is not None else "",
...
dict_to_csv(final_list, "B2B In-Store Ungrouped Report")
